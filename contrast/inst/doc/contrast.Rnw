...
\begin{figure}[p]
   \begin{center}		
<<example1Plot, fig = TRUE, echo = FALSE>>=
   trellis.par.set(col.whitebg(), warn = FALSE)
   ...
   out <- stripplot(
      ...
      function(x, y, groups, ...)
      {   
         groupvalues <- sort(unique(factor(groups)))
         for (i in seq(along=groupvalues)) 
         {
            id <- (groups == groupvalues[i])               
            panel.xyplot(jitter(as.numeric(x[id]), factor = .25), y[id], 
               col = plotTheme$superpose.symbol$col[i],
               pch = plotTheme$superpose.symbol$pch[i], ...)
            panel.xyplot(as.numeric(unique(sort(x[id]))), tapply(y[id], x[id], mean), 
               col = plotTheme$superpose.symbol$col[i], type = "l")
         }
      },
...
\begin{figure}[p]
   \begin{center}		
<<example2Plot, fig = TRUE, echo = FALSE>>=
   trellis.par.set(col.whitebg(), warn = FALSE)
      ...
      {   
         ...
         {       
            ...
            panel.xyplot(jitter(as.numeric(x[id]), factor = .25), y[id], 
               col = plotTheme$superpose.symbol$col[i],
               pch = plotTheme$superpose.symbol$pch[i], ...)
            groupMeans <- tapply(y[id], x[id], mean)
            if(sum(!is.na(groupMeans)) > 1)
            {
               panel.xyplot(as.numeric(unique(sort(x[id]))), groupMeans, 
                  col = plotTheme$superpose.symbol$col[i], type = "l")
            }
         }
      },
...
{\small
<<design2lme>>=
   geeFit <-  geese(2^expression ~ group, data = example2, id = subject, family = gaussian(link = "log"), corstr = "exchangeable")
   summary(geeFit)
...
setwd("/Users/kuhna03/Documents/Code/contrast/inst/doc/")
options(width = 80)

Sweave(
   "/Users/kuhna03/Documents/Code/contrast/inst/doc/contrast.Rnw",
   output = "/Users/kuhna03/Documents/Code/contrast/inst/doc/contrast.tex")
